...
export default function HandsShop({ setCurrentShopItem, melee, pistol, main, explosives, packs, skins, bodies, hands, emojis }) {
    ...
    function checkObtained(){
        hands.map(hand => {
            handShopItems.map(item => {
                if (hand.name === item.name && hand.unlocked) {
                    item.obtained = true;
                }
            })
        })
    ...
    function buy(item, i) {
        if (!item.obtained && +JSON.parse(localStorage.getItem(item.priceType)) >= item.price) {
            hands.push({name: item.name, src: item.src, rank: item.rank, unlocked: true});
            const newWeapons = [{
                melee: melee,
                pistol: pistol,
                main: main,
                ...
                <div className="hands-shop item-shop">
                    ...
                    <div className="shop-cards-container">
                        <div className="shop-cards">
                            {handShopItems.map((item, i) => (
                                <div className={"shop-card hand-card " + item.obtained} onClick={() => buy(item, i)}>
